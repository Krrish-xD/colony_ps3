...
resource = Resource.create({"service.name": "catalog"})
...
logger = logging.LoggerAdapter(logger, extra={"service": "catalog"})
...
@app.get("/")
async def root():
    logger.info("Handling root request for catalog")
    ...
    return {"service": "catalog", "downstreams": responses}
