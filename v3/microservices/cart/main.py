...
logger = logging.LoggerAdapter(logger, extra={"service": "cart"})

app = FastAPI(title="cart Service", version="3.0.0")
...
@app.get("/")
async def root():
    logger.info("Handling root request for cart")
    ...
    return {"service": "cart", "downstreams": responses}
