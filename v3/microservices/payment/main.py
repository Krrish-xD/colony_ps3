...
logger = logging.LoggerAdapter(logger, extra={"service": "payment"})

app = FastAPI(title="payment Service", version="3.0.0")
...
@app.get("/")
async def root():
    logger.info("Handling root request for payment")
    simulate_latency_and_errors()
    
    # downstream simulation
    downstreams = ["notification"]
    responses = []
    ...
    return {"service": "payment", "downstreams": responses}
