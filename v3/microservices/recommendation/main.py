...
resource = Resource.create({"service.name": "recommendation"})
...
logger = logging.LoggerAdapter(logger, extra={"service": "recommendation"})
...
@app.get("/")
async def root():
    logger.info("Handling root request for recommendation")
    ...
    return {"service": "recommendation", "downstreams": responses}
