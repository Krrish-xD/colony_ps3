...
export default function ActionLog() {
  return (
    <section className="bg-black border border-cyber-dim p-4 rounded-md mt-6 h-64 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto font-mono text-sm space-y-1 mt-2">
        {mockLogs.map((log, idx) => (
          <div key={idx} className="flex">
            <span className="text-cyber-dim mr-3">[{log.time}]</span>
            <span className={`${log.color} font-bold mr-2 w-28`}>{log.level}</span>
